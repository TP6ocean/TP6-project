...
class Slickr_Flickr_Plugin {
	...
	const CACHE_EXPIRY = 43200;	
 	...
 	private $changelog = SLICKR_FLICKR_CHANGELOG;
	...
	public function init() {
		...
		$this->message = new Slickr_Flickr_Message();
		$this->newsfeeds = apply_filters('slickr_flickr_news',$this->newsfeeds);

 		$this->maybe_load_bundled_simplepie();
        if (is_admin()) 
            $this->admin_init(); 
        else 
            $this->public_init();
   ...
   private function activation_key_name() { 
    ...
    public function maybe_load_bundled_simplepie() {
        require ABSPATH . WPINC . '/version.php';    
        if ((version_compare($wp_version, '5.6') < 0)
        && (!class_exists('SimplePie')))
         require_once($d . 'simplepie/autoloader.php'); //Load bundled version of SimplePie if running older version of WordPress
    }
