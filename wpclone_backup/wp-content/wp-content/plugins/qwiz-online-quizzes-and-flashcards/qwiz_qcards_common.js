      ...
      $.ajax ({type:    'POST',
               ...
               error:   function (XMLHttpRequest, statusText, errorThrown) {
                           ...
                           if (url.indexOf ('record') != -1) {
                              ...
                              // Called from record_response_v3.php or
                              // record_qcard_v2.php.
                              if (qname == 'qwiz_') {
...
this.check_session_id = function (i_qwiz) {
   ...
   if (! cookie_session_id) {
      ...
      // Set it for now -- others will be checking -- (e.g., record_response_v3).
...
this.set_user_menus_and_icons = function (sign_out_f, bounce_all) {
   ...
   // Note: "qname" and "i_qwiz" replaced below with appropriate values before
   // htm inserted into page for each quiz/deck.
   ...
   if (document_qwiz_user_logged_in_b === true) {
      ...
      */
      ...
      if (! sign_out_f && ! document_qwiz_declined_login_b) {
         ...
         // Always bounce if not logged in.
...
this.create_use_dataset_options2 = function (qname, i_qwiz, dataset, qrecord_id) {
   ...
   // Default callback is create_use_dataset_options2 ().
...
this.create_use_dataset_options3 = function (data, qname, i_qwiz) {
   ...
   if (! units_all_other || ! unit_topics_all_other['Other']) {
      ...
      // If only distinction is topics, set flag to expand view.
      if (units_all_other) {
      ...
      for (var i=0; i<n_units_topics; i++) {
         ...
         if (unit != prev_unit) {
            ...
            // Column: expand/collapse unit icon.  Starts out as expand.  None
            // if topics are all "Other".
         ...
         // Selection checkbox.  Don't show if topics_only_f set.
      ...
      // Nothing but "Other".  Get and show totals.
   ...
   // Expand all if topics_only_f set or if flag set (because only unit is
   // "Other").
...
function get_dataset_units_topics (qname, i_qwiz, qrecord_id, dataset, callback) {
   ...
   // Default callback is create_use_dataset_options3 ().
   if (callback) {
...
this.taker_registered = function (data) {
   ...
   if (! errmsg) {
      ...
      if (register_taker_global_reg_code_ok_f) {
         ...
         // Had reg code; add to class.  If existing user logged in, didn't
         // enter first name or last name -- but these are returned by
         // new_student_registration.php.
         if (data.login_f) {
...
this.remove_tags_eols = function (htm, keep_img_f) {
   if (htm) {
      if (keep_img_f) {
         ...
         // Kludge - reset to non-tag, then undo later.
...
this.replace_smart_quotes = function (string) {
   ...
   // And replace en dash with regular hyphen.
...
this.get_dataset_questions = function (dataset, qname, i_qwiz_qdeck, qrecord_id,
   ...
   if (use_dataset_questions_htm) {
      ...
      // dataset-question IDs, pass to get_dataset_questions_v3.php.
      ...
      // If from qwizard, flag to use maker session ID to retrieve secure quiz
      // questions in case locked.
...
this.receive_and_place_dataset_questions = function (response) {
   ...
   // or from get_dataset_questions_v3.php (when via qjax).
   ...
   // If timed out, leave it (don't want conflict with "Try again" button
   // press.)
      ...
      if (errmsg) {
         if (qqc.isInteger (errmsg)) {
            ...
            if ((! dataset_intro_f || dataset_intro_f == 'topics_only')
               ...
               // Do first, so dataset_intro_info_only_msg gets set.
   ...
   if (qname == 'qwiz_') {
      ...
      if (use_dataset_questions_f == 1) {
         ...
         // Questions separated by '[qQq]' (per get_dataset_questions_v3.php).  Split into array.
         // Take out separator.
      ...
      // If a code highlighter is present, initialize in case any code in
      // dataset.
      if (typeof Prism != 'undefined') {
         Prism.highlightAllUnder ($questions[0]);
      ...
      // Go to first question.  next_question_from_intro () resets "Start
      // quiz" to "Next question" and sets back-forward.
      // Wait till after reset unit/topic questions-done counts if flag set
      // (resuming in-progress quiz).
...
this.restore_topic_question_counts = function (data) {
   ...
   // Go to first question.  next_question_from_intro () resets "Start quiz" to
   // "Next question" and sets back-forward.
   qwiz_.set_qwizdata (i_qwiz_qdeck, 'login_show_b', false);
   qwiz_.set_qwizdata (i_qwiz_qdeck, 'i_question', -1);
   qwiz_.next_question (i_qwiz_qdeck, true, true);
   qwiz_.next_question_from_intro (i_qwiz_qdeck, 0);
...
function insert_dataset_questions (htm, questions_html, qname) {
   ...
   // get_dataset_questions_v3.php).  Split into array.
   ...
   for (var ii=0; ii<n_quizzes_decks; ii++) {
      ...
      if (pay_errmsg) {
         ...
         for (var i=0; i<n_pay_options; i++) {
            ...
            // logged-in student hasn't paid or license expired).  Exclude
            // options with good-through date in future -- student has already
            // signed up for those.
...
//    const b64         = base64js.fromByteArray (binary_data);
//    const binary_data = base64js.toByteArray (b64);
//    const len         = base64js.byteLength (b64);
