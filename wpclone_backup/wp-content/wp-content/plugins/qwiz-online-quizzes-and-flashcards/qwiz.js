...
$ (document).ready (function () {
   ...
   incorrect = [T ('Sorry, no.'), T ('No, that\'s not correct.'), T ('Sorry, that\'s not correct.')];
...
this.display_question = function (i_qwiz, i_question, start_quiz_b) {
   ...
   if (qwizzled_b) {
      ...
      if (q.qwizard_b) {
         $hotspot_image_stack.find ('img.layer0_edited').addClass ('qwizard_display_block_important');
         ...
         spotmap_size = Math.sqrt (spotmap.length);
         hotspot_image_width  = $canvas.attr ('width');
         hotspot_image_height = $canvas.attr ('height');
         if (qwizdata[i_qwiz].answered_correctly[i_question] == -1) {
            ...
            const hotspot_labels = $hotspot_image_stack
         ...
         current_query_hotspot_no = pick_random_hotspot (i_qwiz, i_question);
         set_hotspot_label_query ($qwizq);
         $hotspot_image_stack.off ('click');
         $hotspot_image_stack.on ('click', qwiz_.hotspot_diagram_click);
      }
   }
...
function process_hotspot_diagram (i_qwiz, i_question, question_htm, opening_tags) {
   ...
   if (q.qwizard_b) {
      var hotspot_image_pos = question_htm.indexOf ('<div class="hotspot_image_stack"');
      var editable = question_htm.substr (0, hotspot_image_pos);
   ...
   if (qwizdata[i_qwiz].qrecord_id) {
      if (qqc.isInteger (qwizdata[i_qwiz].dataset_id[i_question])) {
         var question_htm_wo_tags = qqc.remove_tags_eols (htm);
...
this.hotspot_diagram_click = function (e) {
   if (debug[0]) {
      console.log ('[hotspot_diagram_click] event.offsetX:', event.offsetX, ', event.offsetY:', event.offsetY);
      console.log ('[hotspot_diagram_click] current_query_hotspot_no:', current_query_hotspot_no);
   }
   const $qwizq = $ (e.currentTarget).parents ('div.qwizq');
   const i_qwiz = $qwizq.parents ('div.qwiz')[0].id.substr (4);
   const i_question = qwizdata[i_qwiz].i_question;
   if (qwizdata[i_qwiz].user_question_number == 1) {
      $ ('div#icon_qwiz' + i_qwiz).hide ();
      alert_not_logged_in (i_qwiz);
   }
   const $hotspot_label = $qwizq.find ('div.qwiz_hotspot_label.qwiz_hotspot' + current_query_hotspot_no);
   var hotspot_nos = qwizdata[i_qwiz].hotspot_nos[i_question];
   var ii = -1;
   for (var j=0; j<n_hotspots; j++) {
      if (Math.abs (hotspot_nos[j]) == current_query_hotspot_no) {
         ii = j;
         break;
      }
   }
   ...
   var feedback_msg;
   const click_hotspot_no = click_is_hotspot (event.offsetX, event.offsetY);
   if (click_hotspot_no == current_query_hotspot_no) {
      hotspot_nos[ii] += Math.sign (hotspot_nos[ii]) * 1000;
      qwizdata[i_qwiz].n_hotspots_to_do[i_question]--;
      feedback_msg = $hotspot_label.find ('span.hotspot_label_feedback_correct_content').html ();
      ...
      if (qwizdata[i_qwiz].n_hotspots_to_do[i_question]) {
         feedback_msg += '&emsp;<b>Now click on:</b>';
         current_query_hotspot_no = pick_random_hotspot (i_qwiz, i_question);
         set_hotspot_label_query ($qwizq);
      } else {
         set_hotspot_label_query ($qwizq, true);
         $hotspot_image_stack.off ('click');
         if (! q.qwizard_b && ! q.preview) {
            const correct_b = qwizdata[i_qwiz].answered_correctly[i_question] == 1;
            if (correct_b) {
               qwizdata[i_qwiz].n_correct++;
            } else {
               qwizdata[i_qwiz].n_incorrect++;
      ...
      qwizdata[i_qwiz].answered_correctly[i_question] = -1;
      feedback_msg = $hotspot_label.find ('span.hotspot_label_feedback_incorrect_content').html ();
      if (qqc.is_only_tags_and_whitespace (feedback_msg)) {
      ...
      if (qwizdata[i_qwiz].n_hotspots_to_do[i_question] > 1) {
         ...
         feedback_msg += '&emsp;<b>Please try again:</b>';
      }
      current_query_hotspot_no = pick_random_hotspot (i_qwiz, i_question);
      set_hotspot_label_query ($qwizq);
   }
...
function click_is_hotspot (ix, iy) {
   const sx = Math.round (ix/hotspot_image_width  * spotmap_size);
   const sy = Math.round (iy/hotspot_image_height * spotmap_size);
   const hotspot_no = spotmap[sy*spotmap_size + sx];
   if (debug[0]) {
      console.log ('[click_is_hotspot] ix:', ix, ', iy:', iy, ', hotspot_no:', hotspot_no);
   }
   return hotspot_no;
}
...
function set_hotspot_label_query ($qwizq, hide_f) {
